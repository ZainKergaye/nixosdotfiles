  ...
  config = lib.mkIf config.hyprland-hm-config.enable {
    ...
    programs.waybar = {
      ...
      settings.mainBar = {
        ...
        cpu = {
          format = "  {usage}%";
          "on-click" = lib.getExe pkgs.btop;
          tooltip = true;
          interval = 1;
        };

        memory.format = "  {used:0.2f}G";
        ...
        "custom/logo" = {
          ...
          tooltip = false;
          "on-click" =
            "${lib.getExe pkgs.chromium} --new-window 'https://search.nixos.org/packages?channel=unstable&size=50&sort=relevance&type=packages&query=+'";
          "on-click-middle" = "${lib.getExe pkgs.kitty} --hold ${lib.getExe pkgs.fastfetch}";
          "on-click-right" = lib.getExe pkgs.hyprsysteminfo;
        };
